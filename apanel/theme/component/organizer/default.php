...
<?endif;?>
<h3>ორგანაიზერი</h3>
<a href="/apanel/index.php?component=organizer&section=add" class="btn-green">ბანერის შეთავაზება</a>
<a href="/apanel/index.php?component=organizer&section=other" style="padding:7px 15px;cursor:pointer;font-family:BPGMrgvlovani;background-color:purple;max-width:140px;float:right;border-radius:5px;color:#FFF;text-align:center">სხვა შეთავაზებები</a>
<div class="fix"></div>
<form action="/apanel/index.php" method="get">
    ...
    <table class="formadd" width="100%">
        ...
        <tr>
            ...
            <td>თარიღი</td>
            <td>
                <input type="text" name="from" value="<?=$_GET['from'];?>" class="calendar input_80" placeholder="დან"/>
            </td>
            <td width="10"></td>
            <td>
                <input type="text" name="to" value="<?=$_GET['to'];?>" class="calendar input_80" placeholder="მდე"/>
            </td>
            <td width="10"></td>
            <td>კომპანია</td>
            <td>
                <input type="text" value="<?=$_GET['company'];?>" name="company" style="width:100px;"/>
            </td>
            <td width="10"></td>
...
<table id="rounded-corner">
    ...
    <tr>
        ...
        <th scope="col" class="rounded">ელფოსტა</th>
        <th scope="col" class="rounded">რუბრიკა</th>
        <th scope="col" class="rounded">პოზიცია</th>
        <th scope="col" class="rounded">ზომა</th>
        <th scope="col" class="rounded">შენიშვნა</th>
        <th scope="col" class="rounded">დაკავშირების თარიღი</th>
    ...
    <?if(count($registry['banners']) > 0): $ready = array();?>
        <?foreach($registry['banners'] as $item): $info = unserialize($item['info']);?>
            <tr  <?if($item['status'] == '1'):?>style="background:green;"<?elseif($item['status']=='2'):?>style="background:red;"<?else:?><?endif;?>>
                ...
                <td><?=date('Y-m-d',strtotime($item['contact_at']));?> <?=(date('Y-m-d',strtotime($item['contact_at'])) == date('Y-m-d')) ? 'დღეს' : '';?> <?=(date('Y-m-d',strtotime($item['contact_at'])) == date('Y-m-d',strtotime('+1 days'))) ? 'ხვალ' : '';?> <?=(date('Y-m-d',strtotime($item['contact_at'])) == date('Y-m-d',strtotime('+2 days'))) ? 'ზეგ' : '';?></td>
                <td width="120">
                    <?if($item['status'] == '1'):?>
                        კი
                    ...
                    <?else:?>
                        <div>
                            <a href="/apanel/index.php?component=organizer&section=edit&edit=<?=$item['id'];?>&answer=1" style="display:inline-block;background-color:green;padding:5px 15px;border-radius:5px;color:#FFF;">კი</a>
                            <a href="/apanel/index.php?component=organizer&section=edit&edit=<?=$item['id'];?>&answer=2" style="display:inline-block;background-color:red;padding:5px 15px;border-radius:5px;color:#FFF;">არა</a>
                        </div>
                    <?endif;?>
                </td>
                <td align="center" class="option_icons">
                        <?if(get_access('admin','banners','edit')):?>
                            <a href="/apanel/index.php?component=organizer&section=edit&edit=<?=$item['id'];?>"><img src="<?=$theme_admin?>images/user_edit.png" alt="რედაქტირება" title="რედაქტირება" border="0" /></a>
