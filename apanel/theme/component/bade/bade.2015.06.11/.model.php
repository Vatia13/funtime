    ...
    if($_POST['action'] == 'image'){
        if(isset($_POST['date'])){
            if(intval($_POST['date']) > 0 && intval($_POST['cat']) > 0){
                if($DB->execute("UPDATE #__bade SET img='".$_POST['value']."' WHERE date='".intval($_POST['date'])."' AND cat_id='".intval($_POST['cat'])."' ")){
                    die('success');
                }else{
                    die('wrong');
                }
...
if($_GET['parse']=='json'){
    ...
    if($_POST['start'] && $_POST['end']){
        $registry['rubrics'] = $DB->getAll('select #__category.name,#__category.users,#__category.cat_chpu,#__category.id,#__bade.date,#__bade.img,#__bade.fb
                                            FROM #__category
            ...
            $i=0;foreach($registry['rubrics'] as $item):
                ...
                if(!$news[$item['date']]['id']){
                    ...
                    if($user->get_property('gid') == 19){
                        if($news[$item['date']]['moderate'] == 1){
                            $result[$i]['color'] = 'green';
                            $result[$i]['title'] .= '<input style="width:35px; height:35px;" onClick="checkImage('.$item['date'].','.$item['id'].',jQuery(this),\'facebook\')" type="checkbox" name="fb" value="'.$fb_val.'" '.$fb_checked.'/>';
                        }else{
                            $result[$i]['color'] = 'grey';
                        }
                    }
                ...
                if($news[$item['date']]['moderate'] > 0){
                    if($user->get_property('gid') == 24 or $user->get_property('gid') == 25){
                        if(in_array(25,$group) or in_array(24,$group) or in_array(0,$group)){
                            $checkimg = ($item['img'] > 0) ? '<img src="images/picture.png" style="position:absolute;right:5px;top:0;width:12px;"/>' : '';
                ...
                if($user->get_property('gid')!=18 && $user->get_property('gid')!=19 && $user->get_property('gid')!=20 && $user->get_property('gid') != 23 and $news[$item['date']]['moderate'] <> 1 && $news[$item['date']]['id'] > 0){
