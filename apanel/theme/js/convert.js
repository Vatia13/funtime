...
tinymce.PluginManager.add('newmedia', function(editor, url) {
    var urlPatterns = [
    ...
    function getVideoScriptMatch(src) {
        var prefixes = editor.settings.newmedia_scripts;
    ...
    function showDialog() {
        ...
        if (editor.settings.newmedia_alt_source !== false) {
            generalFormItems.push({name: 'source2', type: 'filepicker', filetype: 'newmedia', size: 40, label: 'Alternative source'});
        }

        if (editor.settings.newmedia_poster !== false) {
            generalFormItems.push({name: 'poster', type: 'filepicker', filetype: 'image', size: 40, label: 'Poster'});
        }

        if (editor.settings.newmedia_dimensions !== false) {
    ...
    function dataToHtml(data) {
        ...
        if (data.embed) {
            ...
            } else if (data.source1mime == "image") {
                html += '<img src="' + data.source1 +'" />';
    ...
    function sanitize(html) {
        if (editor.settings.newmedia_filter_html === false) {
            return html;
        }
