...
var curFieldNameId = ($("#parentId > div").length <= 0) ? 0 : $("#parentId > div").length;

function addFielda(akey) {
    curFieldNameId = ($("#parentId > div").length > 0) ? parseInt($("#parentId > div:last-child").attr('data-num')) + 1 : 0;
    var curFieldNumber = curFieldNameId + 1;
    var div = document.createElement("div");
    $(div).attr('data-num',curFieldNameId).html("<textarea name=\"slide[name][]\" id=\"editor_"+curFieldNameId+"\" class=\"slide_name tinymce2\" placeholder=\"ტექსტი\" style=\"align:left;\"></textarea><input type='hidden' name='slide[img][]' class=\"slide_img\" id='slide" + curFieldNumber + "'/> <br><a href='http://funtime.ge/filemanager/dialog.php?type=1&akey=" + akey + "&field_id=slide" + curFieldNumber + "' class='btn-blue iframe-btn' type='button'>ფოტო " + curFieldNumber + "</a> <label class='slide" + curFieldNumber + "'>---</label> <a onclick=\"return deleteFielda(this)\" href=\"#\" data-id=\""+curFieldNameId+"\" class=\"razdel-bodys-aa\">[X]</a><br><br>");
...
function addSlideFields(e){
    ...
    var fields = ($("#parentId > div").length > 0) ? $("#parentId > div").length : 0;
    if(selected > 0){
        if(selected < fields){
           var remFields = fields - selected;
            for(var i = 0;i < remFields; i++){
                tinymce.execCommand('mceRemoveEditor', false, 'editor_'+$("#parentId > div:last-child").attr('data-num'));
                $("#parentId > div:last-child").remove();
            }
        }else{
            for(var i = fields; i < selected; i++){
                curFieldNumber = i + 1;
                var div = document.createElement("div");
                $(div).attr('data-num',i).html("<textarea name=\"slide[name][]\" id=\"editor_"+i+"\" class=\"slide_name tinymce2\" placeholder=\"ტექსტი\" style=\"align:left;\"></textarea><input type='hidden' name='slide[img][]' class=\"slide_img\" id='slide" + curFieldNumber + "'/> <br><a href='http://funtime.ge/filemanager/dialog.php?type=1&akey=" + akey + "&field_id=slide" + curFieldNumber + "' class='btn-blue iframe-btn' type='button'>ფოტო " + curFieldNumber + "</a> <label class='slide" + curFieldNumber + "'>---</label> <a onclick=\"return deleteFielda(this)\" href=\"#\" data-id=\""+i+"\" class=\"razdel-bodys-aa\">[X]</a><br><br>");
                document.getElementById("parentId").appendChild(div); // Добавляем новый узел в конец списка полей
...
var timer = setInterval(function(){
    if($("#image_dirs").val() != "" && $("#parentId > div").length <= 1 && $("#parentId > div:first-child").attr('data-num') <= 0){
        var akey = "a651481913d2fedc5c880b5f14cb9859";
        var curFieldNumber = 0;
        $.ajax({
            ...
            success:function(response){
                ...
                if(json.img.length > 0){
                    for(var i = 1; i < json.img.length; i++){
                        ...
                        var div = document.createElement("div");
                        $(div).attr('data-num',i).html("<textarea name=\"slide[name][]\" id=\"editor_"+i+"\" class=\"slide_name  tinymce2\" placeholder=\"ტექსტი\" style=\"align:left;\"></textarea><input type='hidden' name='slide[img][]' class=\"slide_img\" value='"+json.img[i]+"' id='slide" + curFieldNumber + "'/> <br><a href='http://funtime.ge/filemanager/dialog.php?type=1&akey=" + akey + "&field_id=slide" + curFieldNumber + "' class='btn-blue iframe-btn' type='button'>ფოტო " + curFieldNumber + "</a> <label class='slide" + curFieldNumber + "'>"+lab[num]+"</label> <a onclick=\"return deleteFielda(this)\" href=\"#\" data-id=\""+i+"\" class=\"razdel-bodys-aa\">[X]</a><br><br>");
                        document.getElementById("parentId").appendChild(div); // Добавляем новый узел в конец списка полей
