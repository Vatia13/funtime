...
<div class="content">
    ...
    <div class="post-image" style="border:25px solid <?=$registry['color']['frame'];?>">
        ...
        <img src="/img/uploads/news/read/<?=$registry['post'][0]['img'];?>" width="465" height="600" alt="<?=$registry['post'][0]['alt_search']?>"/>
    </div>
    <div class="post-title">
        <h1><?=$registry['post'][0]['title'];?></h1>
        <?if(!empty($registry['post'][0]['title_short'])):?><h3><?=$registry['post'][0]['title_short']?></h3><?endif;?>
        <? $slides = unserialize($registry['post'][0]['slide']);if(count($slides['img']) > 1):?>
            <div class="post-slide-message">ფოტოსლაიდი <?=count($slides['img']);?> ფოტო</div>
        <?endif;?>
    </div>
