...
<div class="saknatuna">
    <div class="saklink">
        <a href="/saknatuno-ambebi/"><img src="/<?=$theme?>/images/saknatuna.png"></a>
    </div>
    <ul>
        ...
        <li class="mob">
            <ul>
                <li><a href="http://<?=$_SERVER['SERVER_NAME'];?>/<?=$item['cat_chpu'];?>/<?=$item['chpu'];?>/">
                        <?=$item['title'];?>
                    </a><br><div class="sak-time"><span><?=gedate('l H:i',$item['date']);?> </span>  <span> <?=gedate('d.m.Y',$item['date']);?></span></div></li><li><a href="http://<?=$_SERVER['SERVER_NAME'];?>/<?=$item['cat_chpu'];?>/<?=$item['chpu'];?>/"><img src="<?=substr($item['thumbs'],2);?>" width="100%" alt="<?=$item['title'];?>" title="<?=$item['title'];?>"></a></li>
            </ul>
        </li>
        <?endforeach;?>
    </ul>
