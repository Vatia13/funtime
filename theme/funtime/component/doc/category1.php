            ...
            <ul>
                <?foreach($registry['posts'] as $item):
                    ...
                    <li data-last_id="<?=$item['id'];?>">
                        <a href="http://<?=$_SERVER['SERVER_NAME'];?>/<?=$item['cat_chpu'];?>/<?=$item['chpu'];?>/"><img src="<?=get_thumb($item['thumbs'],'/images/news/prev',558);?>" width="558"/><?if(count($this['slide']['img']) > 1):?><h3 style="background-image:url('/<?=$theme?>images/main_icon.png');background-repeat:no-repeat;background-position:right 10px center;padding:30px 50px 27px 10px;"></h3><?endif;?></a>
    ...
    <?if($registry['deviceType'] == 'computer'):?>
        ...
        <div class="saknatuno-post-list">
            <ul>
                <?foreach($registry['posts'] as $item):?>
                    <li data-last_id="<?=$item['id'];?>">
                        <div class="saknatuno-image"><a href="http://<?=$_SERVER['SERVER_NAME'];?>/<?=$item['cat_chpu'];?>/<?=$item['chpu'];?>/"><img src="<?=get_thumb($item['thumbs'],'/images/news/prev',215);?>" width="215" align="left"/></a></div>
