	...
	$.fn.prettyPhoto = function(pp_settings) {
		...
		*/
		$.prettyPhoto.initialize = function() {
            ...
            if(pp_settings.facebook == true){
              pp_facebook = (isSet) ? jQuery.map(matchedObjects, function(n, i){ if($(n).attr(settings.hook).indexOf(theRel) != -1) return $(n).attr('data-facebook'); }) : $.makeArray($(this).attr('data-facebook'));
            }
		...
		*/
		$.prettyPhoto.open = function(event) {
			...
			if(settings.facebook == true){
                var shareicon = 'fshare.png';
                var shw = '61px';
                var shh = '25px';
                var mtop = '0';
                if(settings.deviceType == 'phone'){
                    shareicon = 'facebook.png';
                    shw = '24px';
                    shh = '24px'
                }
                if(settings.out_of_date == 1){
                    mtop = '-15px';
                }
				$pp_pic_holder.find('.pp_social a').css({width:shw,height:shh,background:'url(/img/icons/'+shareicon+') no-repeat',backgroundSize:shw + ' ' +shh,position:'relative',left:'0',top:mtop,float:'right'}).attr('href',pp_facebook[rel_index]);
			}
			...
			$pp_pic_holder.fadeIn(function(){
				...
				switch(_getFileType(pp_images[set_position])){
					case 'image':
						...
						$pp_pic_holder.find('.pp_close').show(400);
                        ...
                        if(settings.star_rate == true && settings.out_of_date == 0){
                            pp_star_rate = (isSet) ? jQuery.map(matchedObjects, function(n, i){ if($(n).attr(settings.hook).indexOf(theRel) != -1) return $(n).attr('data-star'); }) : $.makeArray($(this).attr('data-star'));
                            $pp_pic_holder.find('.star_container').html(pp_star_rate[rel_index]);
                        }



                        $pp_pic_holder.find('.pp_next').show();
		...
		*/
		function _showContent(){
			...
			$pp_pic_holder.animate({
                ...
                if(pp_settings.deviceType != "phone"){
                // WEB SLIDER
				if($("#fullResImage").width() + 370 > window.innerWidth){
					var contWidth = window.innerWidth - 40;
					var holderWidth = window.innerWidth - 30;
					var containerWidth = window.innerWidth - 420;
					$("#fullResImage").width(containerWidth).css('height','auto','!important');
					var imageHeight = $("#fullResImage").height() - 30;
					var contentHeight = $("#fullResImage").height();
				}else{
					var contWidth = $("#fullResImage").width() + 370;
					var holderWidth = $("#fullResImage").width() + 390;
					var containerWidth = $("#fullResImage").width();
					var imageHeight = $("#fullResImage").height() - 30;
					var contentHeight = $("#fullResImage").height();
				}
				...
				$pp_pic_holder.find('.pp_hoverContainer').height(contentHeight);
				// content conteiner width

				$pp_pic_holder.find('.pp_content_container').width(containerWidth);
				$pp_pic_holder.width(holderWidth);
				$pp_pic_holder.find('.pp_details').height(imageHeight);
		...
		*/
		function _hideContent(callback){
			...
			$pp_pic_holder.find('.pp_close').hide();
		...
		*/
		...
		function _build_overlay(caller){
            ...
            /*
            if(settings.deviceType == 'phone'){
                $pp_pic_holder.find('.pp_content_container #pp_full_res').bind('click',function(){
                    $.prettyPhoto.changePage('next');
                    $.prettyPhoto.stopSlideshow();
                    return false;
                });
