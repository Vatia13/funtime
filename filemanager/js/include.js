...
$(document).ready(function(){
	...
	var val=$(this).val(); //fix_filename($(this).val());
	$(this).val(val);
	delay(function(){
	    if (js_script) {
...
function apply_img(file,external){
  ...
  if (external!=""){
		if ($('#crossdomain').val()==1){
			...
			var target = $('#'+external, window_parent.document);
            var targetc = $('.'+external, window_parent.document);
			target.val(url).trigger('change');
            var lab = url.split('/');
            var num = lab.length - 1;
            targetc.text(lab[num]).trigger('change');
			var imageDir = $('#image_dirs', window_parent.document);
			imageDir.val(url).trigger('change');
			if(typeof window_parent.responsive_filemanager_callback == 'function'){
				window_parent.responsive_filemanager_callback(external);
			}
			close_window();
