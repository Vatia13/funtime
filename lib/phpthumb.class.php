...
//		}
	...
	function RenderToFile($filename) {
		if (preg_match('#^(f|ht)tps?\://#', $filename)) {
	...
	function CleanUpCacheDirectory() {
		...
		if (($this->config_cache_maxage > 0) || ($this->config_cache_maxsize > 0) || ($this->config_cache_maxfiles > 0)) {
			...
			foreach ($AllFilesInCacheDirectory as $fullfilename) {
				if (preg_match('#^phpThumb_cache_#', basename($fullfilename)) && file_exists($fullfilename)) {
	...
	function resetObject() {
		...
		foreach ($class_vars as $key => $value) {
			...
			if (!preg_match('#^(config_|debug|fatalerror)#', $key)) {
				$this->$key = $value;
			}
		}
	...
	function ResolveSource() {
		...
		} elseif (preg_match('#^(f|ht)tps?\://#', $this->sourceFilename)) {
			// URL
	...
	function setCacheDirectory() {
		...
		if (substr($this->config_cache_directory, 0, 1) == '.') {
			if (preg_match('#^(f|ht)tps?\://#', $this->src)) {
	...
	function ResolveFilenameToAbsolute($filename) {
		...
		if ($this->iswindows) {
			$AbsoluteFilename = preg_match_replace('#^'.preg_quote(realpath($this->config_document_root)).'#', realpath($this->config_document_root), $AbsoluteFilename);
		...
		if ($commandline) {
			if ($IMtempfilename = $this->phpThumb_tempnam()) {
				...
				if ($this->ImageMagickSwitchAvailable('thumbnail') && $this->config_imagemagick_use_thumbnail) {
					...
					$IMuseExplicitImageOutputDimensions = preg_match('#image dimensions are zero#', $IMresult_test);
				...
				if (is_array($getimagesize)) {
					...
					if (!preg_match('#('.implode('|', $this->AlphaCapableFormats).')#', $outputFormat)) {
				...
				if ($this->ra) {
					...
					if ($this->ImageMagickSwitchAvailable('rotate')) {
						if (!preg_match('#('.implode('|', $this->AlphaCapableFormats).')#', $outputFormat) || phpthumb_functions::version_compare_replacement($this->ImageMagickVersion(), '6.3.7', '>=')) {
				...
				if (preg_match('#jpe?g#', $outputFormat) && $this->q) {
		...
		if (!isset($domain_is_allowed[$hostname])) {
			...
			foreach ($allowed_domains as $valid_domain) {
				...
				if ($starpos !== false) {
					...
					if (preg_match('#'.$valid_domain.'$#', $hostname)) {
...
//echo $maxwidth.'x'.$maxheight.'<hr>';
	...
	function ExtractEXIFgetImageSize() {
		...
		if (preg_match('#^http:#', $this->src) && !$this->sourceFilename && $this->rawImageData) {
			!$this->SourceDataToTempFile();
		}
	...
	function SetCacheFilename() {
		...
		if ($this->config_cache_default_only_suffix && $this->sourceFilename) {
			...
			if (strpos($this->config_cache_default_only_suffix, '*') === false) {
				...
				preg_match('#(.+)(\.[a-z0-9]+)?$#', basename($this->sourceFilename), $matches);
		...
		} elseif (!$this->config_cache_source_filemtime_ignore_remote && preg_match('#^(f|ht)tps?\://#', $this->src)) {
