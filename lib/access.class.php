...
class flexibleAccess{
  ...
  function flexibleAccess($dbConn = '', $settings = '')
  {
	    ...
	    $this->remCookieDomain = $this->remCookieDomain == '' ? $_SERVER['SERVER_NAME'] : $this->remCookieDomain;
  ...
  function login($uname, $password, $remember = false, $loadUser = true)
  {
		...
		if ( $loadUser )
		{
            ...
            $this->userData['upload_img']=$this->userData['show_img'];
			if($this->userData['gid']==1)$this->userData['gid']=25;
            if($this->userData['gid']==2)$this->userData['gid']=22;
            ...
            if($this->userData['gid']==6)$this->userData['gid']=21;
            if($this->userData['gid']==7)$this->userData['gid']=19;
            if($this->userData['gid']==8)$this->userData['gid']=20;
            if($this->userData['gid']==9)$this->userData['gid']=30;
	header('P3P: CP="CUR ADM"');
	$cookie_name = 'forum_cookie_5110d1';
	$cookie_domain = '';
	$cookie_path = '/';
	$cookie_secure = 0;
	$expire = time() + 5600;//1209600;
  ...
  */
  ...
  {
    setcookie($this->remCookieName, '', time() - $this->remTime, '/', $this->remCookieDomain);

    $cookie_name = 'forum_cookie_5110d1';
	$cookie_domain = '';
	$cookie_path = '/';
	$cookie_secure = 0;
	$expire = time() - 8600;//1209600;
  ...
  */
  ...
  function insertUser($data){
    ...
    foreach ($data as $k => $v ): $data[$k] = "'".$this->escape($v)."'";endforeach;
